# baseline, interpolation, scale, rotation, both
...
def process_split(ske_files, train_set, test_set, mode, label='XSub'):
    x = {'train': [], 'test': []}
    y = {'train': [], 'test': []}
    skipped = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, required=True,
                        choices=['baseline', 'interpolation', 'scale', 'rotation', 'both'],
                        help='Tryb preprocessingu: baseline | interpolation | scale | rotation | both')
    ...
    for benchmark, txt_path, out_stem in [
        ...
        x, y = process_split(ske_files, train_set, test_set, mode=args.mode, label=benchmark)
        ...
        if args.mode == 'both':
            out_name = f'{out_stem}_both_with_rotation.npz'
        else:
            out_name = f'{out_stem}_{args.mode}.npz'        
