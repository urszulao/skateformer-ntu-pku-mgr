...
XSUB_TXT   = BASE / 'cross-subject.txt'
...
SPLIT_TXT = {'xsub': XSUB_TXT, 'xview': XVIEW_TXT}[SPLIT]
OUT_DIR   = BASE / ('output/sweep' if SPLIT == 'xsub' else 'output/sweep_xview')

SWEEP_W = [(32, 8), (64, 16), (96, 24), (128, 32)]
W_BEST   = 64
SWEEP_S_BY_SPLIT = {
    'xsub':  [(W_BEST, 8), (W_BEST, 32)],
    'xview': [(W_BEST, 32)],
}
SWEEP_S = SWEEP_S_BY_SPLIT[SPLIT]

SWEEP_CONFIGS = SWEEP_W + SWEEP_S
...
def main():
    ...
    for W, S in configs:
        ...
        if SKIP_EXISTING and out_path.exists():
            print(f'  W={W:3d} S={S:3d}: plik juz istnieje')
            continue
        ...
        print(f'  W={W:3d} S={S:3d}: {n_total} okien, {gb:.2f} GB')

        x_test = np.empty((n_total, W, 150), dtype=np.float32)
        ...
        assert k == n_total, f'Niezgodnosc: wypelniono {k}, zaalokowano {n_total}'
